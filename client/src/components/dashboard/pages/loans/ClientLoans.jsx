...
const Loans = ({ setAuth }) => {
  ...
  useEffect(() => {
    ...
    if (!token) {
      ...
      GetLoans(); // Fetch loans if token is found
    }
  }, [navigate]);
      ...
      <div className="flex w-full">
        <div className="w-full h-[900px] mx-auto px-8 py-8 mb-4 border bg-white shadow-md rounded">
          <div className="flex items-center justify-between px-4 py-5 sm:px-6 bg-red-500 rounded shadow-md">
            <div>
              <h3 className="text-lg font-medium leading-6 text-white">Loans Report</h3>
              <p className="mt-1 max-w-2xl text-sm text-white">Loans summary and informations.</p>
            </div>
          ...
          <div className="flex items-center justify-between border-y-2 mt-5">
            <h3 className="text-lg font-medium leading-6 text-gray my-2 px-1 py-2">Loan Transactions</h3>
            ...
            <table className="table-fixed text-center">
              <thead>
                <tr>
                  ...
                  <th className="w-1/6 px-4 py-2 text-gray-600">Outstanding Balance</th>
                  ...
                  loans.map((loan, index) => {
                    return (
                      <tr key={index}>
                        ...
                        <td className="border px-4 py-2">{loan.firstname + ' ' + loan.lastname}</td>
                        <td className="border px-4 py-2 bg-gray-50">{loan.type}</td>
                        <td className="border px-4 py-2">₱ {loan.balance}</td>
                        <td className="border px-4 py-2 bg-gray-50">₱ {loan.gross_loan}</td>
                        <td className="border px-4 py-2">₱ {loan.amort}</td>
                        <td className="border px-4 py-2 bg-gray-50">{loan.terms} month/s</td>
                        <td className="border px-4 py-2">{new Date(loan.date_released).toDateString()}</td>
                        <td className="border px-4 py-2 bg-gray-50">
                          {loan.status === 'Approved' || loan.status === 'Fully Paid' || loan.status === 'Disbursed' ? (
                            <span className="bg-green-500 text-white px-4 py-1 rounded-md">{loan.status}</span>
                          ) : loan.status === 'Declined' ? (
                            <span className="bg-red-400 text-white px-4 py-1 rounded-md">{loan.status}</span>
                          ) : (
                            <span className="bg-yellow-300 text-white px-4 py-1 rounded-md">{loan.status}</span>
                          )}
                        </td>
